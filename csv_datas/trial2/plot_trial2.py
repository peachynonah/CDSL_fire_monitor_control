...
def plot_joint_data(file_number, start_time=None, end_time=None):
    base_path = "/home/kiro/CDSL/CDSL_fire_monitor_control/csv_datas/"
    ...
    try:
        ...
        required_cols = {"joint_pos_desired", "joint_pos", "joint_vel_desired", 
                         "joint_vel_filtered", "target_torque", "current_time"}
    ...
    df = df.dropna(subset=["joint_pos_desired", "joint_pos", "joint_vel_desired", 
                           "joint_vel_filtered", "target_torque", "current_time"])
    ...
    # Target Torque
    axs[2].plot(df["current_time"].to_numpy(), df["target_torque"].to_numpy(), label="target_torque", color="tab:purple")
    axs[2].set_xlabel("time")
    axs[2].set_ylabel("torque")
    axs[2].set_title("Target Torque (time)")
    axs[2].grid(True)
    axs[2].legend()
